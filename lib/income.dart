...
class IncomeList extends StatelessWidget {
  ...
  const IncomeList({required this.incomeItems});
    ...
    return Container(
      ...
      child: Column(
        ...
        children: [
          ...
          SizedBox(height: 5),
          buildIncomeListView(screenWidth),
        ],
      ),
    );
  }
  ...
  Widget buildIncomeListView(double screenWidth) {
    return SingleChildScrollView(
      scrollDirection: Axis.vertical, // Change scroll direction to vertical
      child: Container(
        width: calculateListWidth(incomeItems, screenWidth),
        child: Column(
  ...
  double calculateListWidth(List<IncomeItem> items, double screenWidth) {
    // Calculate the total width required for all items
    double totalWidth = 0;
    for (IncomeItem item in items) {
  ...
  double calculateItemWidth(IncomeItem item, double screenWidth) {
  ...
  Widget buildIncomeItem(IncomeItem item, double screenWidth) {
    return Row(
  ...
  Widget buildRelationAndUserName(IncomeItem item, double screenWidth) {
    return Column(
...
void main() {
  runApp(MaterialApp(
    home: Scaffold(
      body: IncomeList(
        incomeItems: generateIncomeData(),
      ),
    ),
  ));
}

List<IncomeItem> generateIncomeData() {
  return [
    IncomeItem(
      isChecked: false,
      relation: 'Friend',
